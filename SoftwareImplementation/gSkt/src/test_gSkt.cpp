...
int main()
{
    ...
    uint32_t mem = 1000;
    gSkt<HLL> gSkt(mem);           //  use HLL as basic Estimator
    // gSkt<Bitmap> gSkt(mem);     //  use Bitmap as basic Estimator    
    
    string flowID, elemID;
    while(int status = filehandler.get_item(flowID, elemID)){
        gSkt.process_packet(flowID, elemID);
    ...
    uint32_t ans = gSkt.get_flow_cardinality(flow_query);
#endif
...
//     string ofile_path = "../../gSkt/SuperSpreader/" + dataset + "/";
//     ifstream ifile_hand;
//     ofstream ofile_hand;
